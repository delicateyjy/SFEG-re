"""
原输出logits，现已增加sigmoid
原输入图像为224x224，是医学CT图好像，直接改成512x512？
训练配置：cosineLR，epochs = 2000？early_stopping_patience = 50
learning_rate = 1e-3，batch_size = 4（作者用的48G显卡，暂时修改为1）
损失函数为BCE+Dice
2000个epoch，第一次见，因为数据集特别小，一个只有几十张
代码比较老，用的python2写的？
"""
...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, config, patchsize, img_size1, img_size2,  in_channels):
        super().__init__()
        # img_size = _pair(img_size)
        patch_size = _pair(patchsize)
        n_patches = (img_size1 // patch_size[0]) * (img_size2 // patch_size[1])
...
"""
修改 __init__ 函数，增加 h_patch 和 w_patch 参数
"""
class Reconstruct(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, scale_factor, h_patch, w_patch):
    ...
    def forward(self, x):
        ...
        # h, w = int(2*np.sqrt(n_patch/6)), int(3*np.sqrt(n_patch/6))
        # h, w = int(3 * np.sqrt(n_patch / 12)), int(4 * np.sqrt(n_patch / 12))
        h, w = self.h_patch, self.w_patch
        assert n_patch == h * w, "Reconstruction error: n_patch does not match h*w."
        x = x.permute(0, 2, 1)
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size1, img_size2, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        super().__init__()
        ...
        # 新添加代码，自动计算patch
        h_patch_1, w_patch_1 = img_size1 // self.patchSize_1, img_size2 // self.patchSize_1
        h_patch_2, w_patch_2 = (img_size1 // 2) // self.patchSize_2, (img_size2 // 2) // self.patchSize_2
        h_patch_3, w_patch_3 = (img_size1 // 4) // self.patchSize_3, (img_size2 // 4) // self.patchSize_3
        h_patch_4, w_patch_4 = (img_size1 // 8) // self.patchSize_4, (img_size2 // 8) // self.patchSize_4

        self.embeddings_1 = Channel_Embeddings(config, self.patchSize_1, img_size1, img_size2, in_channels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(config, self.patchSize_2, img_size1//2, img_size2//2,
                                               in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(config, self.patchSize_3, img_size1//4, img_size2//4,
                                               in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(config, self.patchSize_4, img_size1//8, img_size2//8,
        ...
        self.reconstruct_1 = Reconstruct(channel_num[0], channel_num[0], kernel_size=1,
                                         scale_factor=(self.patchSize_1, self.patchSize_1),
                                         h_patch=h_patch_1, w_patch=w_patch_1)
        self.reconstruct_2 = Reconstruct(channel_num[1], channel_num[1], kernel_size=1,
                                         scale_factor=(self.patchSize_2, self.patchSize_2),
                                         h_patch=h_patch_2, w_patch=w_patch_2)
        self.reconstruct_3 = Reconstruct(channel_num[2], channel_num[2], kernel_size=1,
                                         scale_factor=(self.patchSize_3, self.patchSize_3),
                                         h_patch=h_patch_3, w_patch=w_patch_3)
        self.reconstruct_4 = Reconstruct(channel_num[3], channel_num[3], kernel_size=1,
                                         scale_factor=(self.patchSize_4, self.patchSize_4),
                                         h_patch=h_patch_4, w_patch=w_patch_4)
...
class UCTransNet(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size1=512, img_size2=512, vis=False):
        super().__init__()
        self.vis = vis
        ...
        self.mtc = ChannelTransformer(config, vis, img_size1, img_size2,
...
def test():
    ...
    model = UCTransNet(config_vit, img_size1=512, img_size2=512)
