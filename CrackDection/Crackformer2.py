"""
batchsize为1，使用shuffle，使用SGD，momentum=0.9
总共训练500个epoch，初始学习率为1e-3，使用StepLR调度，
在20,50,100epoch降低1/10学习率
输出为sigmoid后的概率图
"""
import torch
...
class crackformer2(nn.Module):

    def __init__(self):
        super(crackformer2, self).__init__()
...
class crackformer2_loss(nn.Module):
    def __init__(self):
        super(crackformer2_loss, self).__init__()
...
# class crackformer2_loss(nn.Module):
#     def __init__(self, eta=0.2, lmbda=1.1, side_weights=None, fuse_weight=1.0):
#         super(crackformer2_loss, self).__init__()
#         self.eta = eta
#         self.lmbda = lmbda
#         if side_weights is None:
#             self.weights = [1.0] * 5 + [fuse_weight]
#         else:
#             self.weights = side_weights + [fuse_weight]
#     def forward(self, preds, mask):
#         pos_mask = (mask > self.eta).float()
#         neg_mask = (mask <= 0.05).float()
...
#         beta = n_neg / n_total
#         alpha = self.lmbda * n_pos / n_total
...
#             loss_pos = -beta * torch.log(pred)
#             loss_neg = -alpha * torch.log(1 - pred)
#             bce_loss = torch.sum(loss_pos * pos_mask) + torch.sum(loss_neg * neg_mask)
            
#             total_loss += self.weights[i] * bce_loss
#         return total_loss

def test():
    x = torch.randn((1, 3, 512, 512)).cuda()  # batch size, channel,height,width
    model = crackformer2().cuda()
