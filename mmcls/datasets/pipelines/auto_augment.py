...
def merge_hparams(policy: dict, hparams: dict):
    ...
    参数：
        policy (dict)：原始策略配置字典。
        hparams (dict)：需要合并的超参数。

    返回：
        dict：添加 ``hparams`` 后的策略配置字典。
    """
...
class AutoAugment(object):
    ...
    这种数据增强方法在 `AutoAugment: Learning Augmentation
    Policies from Data <https://arxiv.org/abs/1805.09501>`_ 中提出。

    参数：
        policies (list[list[dict]]): 自动增强的策略。 ``policies`` 中的每个
            策略都是一个特定的增强策略，由多个增强（dict）组成。当调用
            AutoAugment 时，将随机选择 ``policies`` 中的一个策略来
            增强图像。
        hparams (dict): 超参数的配置。如果策略字典中未设置这些参数，则在
            需要这些参数的策略中将使用超参数。这默认为使用 _HPARAMS_DEFAULT。
    """
...
class RandAugment(object):
    ...
    此数据增强方法在 `RandAugment: Practical automated
    data augmentation with a reduced search space
    <https://arxiv.org/abs/1909.13719>`_ 中提出。

    参数：
        policies (list[dict]): 随机增强的策略。``policies`` 中的每个
            策略都是一个特定的增强策略（字典）。
            策略至少应具有 `type` 键，指示增强的类型。对于那些具有幅度的策略，
            （考虑到它们在不同增强中命名不同，）
            `magnitude_key` 和 `magnitude_range` 应分别是幅度
            参数（字符串）和幅度范围（格式为
            (val1, val2) 的元组）。请注意 val1 不一定小于 val2。
        num_policies (int): 每次从策略中选择的策略数量。
        magnitude_level (int | float): 所有选择的增强的幅度级别。
        total_level (int | float): 幅度的总级别。默认为 30。
        magnitude_std (Number | str): 应用的幅度噪声的偏差。

            - 如果是正数，幅度从正态分布中抽样
              （均值=magnitude，标准差=magnitude_std）。
            - 如果是 0 或负数，幅度保持不变。
            - 如果是字符串 "inf"，幅度从均匀分布中抽样
              （范围=[min, magnitude]）。
        hparams (dict): 超参数的配置。如果这些参数在策略字典中未设置，
            将在需要这些参数的策略中使用超参数。默认使用 _HPARAMS_DEFAULT。

    注意：
        `magnitude_std` 将为策略引入一些随机性，由
        https://github.com/rwightman/pytorch-image-models 修改。

        当 magnitude_std=0 时，我们按如下方式计算幅度：
...
class Shear(object):
    ...
    参数：
        magnitude (int | float)：用于剪切的幅度。
        pad_val (int, Sequence[int])：用于常量填充的像素 pad_val 值。
            如果是长度为 3 的序列，则分别用于填充 R、G、B 通道。默认为 128。
        prob (float)：执行剪切的概率，应在 [0, 1] 范围内。默认为 0.5。
        direction (str)：剪切方向。选项为“水平”和“垂直”。默认为“水平”。
        random_negative_prob (float)：将幅度变为负数的概率，应在 [0,1] 范围内。默认为 0.5。
        interpolation (str)：插值方法。选项为“最近邻”、“双线性”、“三次卷积”、“区域”、“Lanczos”。默认为“三次卷积”。
    """
...
class Translate(object):
    ...
    参数：
        magnitude (int | float)：用于平移的幅度。请注意，偏移量是通过幅度 * 在相应方向上的大小计算的。幅度为1时，整个图像将被移出范围。
        pad_val (int, Sequence[int])：用于常量填充的像素pad_val值。如果是长度为3的序列，则分别用于填充R、G、B通道。默认为128。
        prob (float)：执行平移的概率，因此应在[0, 1]范围内。默认为0.5。
        direction (str)：平移方向。选项为'水平'和'垂直'。默认为'水平'。
        random_negative_prob (float)：将幅度转为负值的概率，应该在[0,1]范围内。默认为0.5。
        interpolation (str)：插值方法。选项有'最近邻'、'双线性'、'三次插值'、'区域'、'兰索斯'。默认为'最近邻'。
    """
...
class Rotate(object):
    ...
    参数：
        angle (float): 用于旋转的角度。正值表示顺时针旋转。
        center (tuple[float], 可选): 源图像中旋转的中心点 (w, h)。如果为 None，将使用图像的中心。默认为 None。
        scale (float): 等向缩放因子。默认为 1.0。
        pad_val (int, Sequence[int]): 用于常量填充的像素 pad_val 值。如果是长度为 3 的序列，将分别用于填充 R、G、B 通道。默认为 128。
        prob (float): 执行旋转的概率，因此应在 [0, 1] 范围内。默认为 0.5。
        random_negative_prob (float): 将角度变为负数的概率，应该在 [0,1] 范围内。默认为 0.5。
        interpolation (str): 插值方法。选项有 'nearest'、'bilinear'、'bicubic'、'area'、'lanczos'。默认为 'nearest'。
    """
...
class AutoContrast(object):
    ...
    参数：
        prob (float)：执行反转的概率，因此应该在范围 [0, 1] 内。默认为 0.5。
    """
...
class Invert(object):
    ...
    参数：
        prob（浮动）：执行反转的概率，因此应在 [0, 1] 范围内。默认为 0.5。
    """
